import { applyRefDocs, RefDocFound, REF_INDENTIFIER, translateDocFromFirestore, translateDocToFirestore } from "../src/misc";
...
describe("reference-document-parser.spec", () => {
  ...
  test("test translateDocFromFirestore", () => {
    ...
    expect(result.refdocs.length).toBe(1);
    expect(result.refdocs[0].fieldPath).toBe(
      "myrefdoc"
    );
