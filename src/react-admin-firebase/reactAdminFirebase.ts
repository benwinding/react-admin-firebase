...
class FirebaseClient {
  ...
  constructor(
    ...
    this.pathsArray.forEach((inputPath) => {
      ...
      observable.subscribe((querySnapshot: firebase.firestore.QuerySnapshot) => {
        const newList = querySnapshot.docs.map((doc: firebase.firestore.QueryDocumentSnapshot) => {
          return {
            ...doc.data(),
            id: doc.id
          } 
  ...
  public getList(resourceName: string): Promise<Array<{}>> {
    return this.tryGetResource(resourceName)
      .then((resource: IResource) => resource.list)
   }

  public getTotal(resourceName: string): Promise<number> {
    return this.tryGetResource(resourceName)
      .then((resource: IResource) => resource.list.length)
  }

  private setList(newList: Array<{}>, resourceName: string): Promise<any> {
    return this.tryGetResource(resourceName)
      .then((resource: IResource) => {resource.list = newList})
  }
  ...
  private getCollectionObservable(collection: firebase.firestore.CollectionReference): Observable<any> {
...
async function providerApi(type: string, resourceName: string, params: {}) {
  switch(type) {
    case GET_MANY:
    case GET_MANY_REFERENCE:
    case GET_LIST:
      return { data: await fb.getList(resourceName), total: await fb.getTotal(resourceName) };
    case GET_ONE:
    case CREATE:
    case UPDATE:
    case DELETE:
